...
class TestIt(vmtest.VmTestCase):
    ...
    def test_access_variable_in_closure_from_a_class(self):
        self.assert_ok("""\
            def foo():
                x = 1
                class Bar:
    ...
    @pytest.mark.slow
    def test_access_numerous_variables(self):
        # case for `EXTENDED_ARG`
        num = 2**16 + 1
        str_vars = ', '.join(['var%s' % v for v in range(num)])

        g = globals()
        for i in range(num):
            exec('var%s = %d' % (i, i), g, g)
